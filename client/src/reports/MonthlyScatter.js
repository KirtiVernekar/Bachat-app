...
import { plotTransactions } from '../transaction/api-transaction'
...
const MonthlyScatter = ({transactionType}) => {
    ...
    useEffect(() => {
        ...
        plotTransactions({month: month, type: transactionType},{t: jwt.token}, signal).then((data) => {
          if (data.error) {
            setError(data.error)
          } else {
            setPlot(data)
    ...
    const handleDateChange = date => {
        setMonth(date)
        plotTransactions({month: date, type: transactionType},{t: jwt.token}).then((data) => {
          if (data.error) {
            setError(data.error)
          } else {
            setPlot(data)
    ...
    return (
      ...
      <Typography variant="h6" className={classes.title}>Your {transactionType} scattered over </Typography> 
